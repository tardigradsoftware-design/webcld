...
export function Footer() {
  ...
  return (
    <footer className="relative overflow-hidden border-t border-white/10 bg-brand-navy-950 text-white">
      ...
      />
      <div
        className="pointer-events-none absolute -right-32 bottom-0 h-[380px] w-[380px] rounded-full bg-blue-500/15 blur-[130px]"
        aria-hidden
      />
      ...
      <div className="container relative py-14 lg:py-20">
        <div className="grid gap-12 lg:grid-cols-[1.4fr_1fr_1fr_1fr]">
          ...
          <div>
            ...
            <ul className="mt-6 space-y-3 text-sm">
              ...
              <li className="flex items-start gap-3">
                <Mail className="mt-0.5 h-4 w-4 shrink-0 text-blue-300" aria-hidden />
                <a
                  href={`mailto:${CONTACT.email}`}
                  className="text-white/75 transition hover:text-blue-300"
                >
                  {CONTACT.email}
                </a>
              </li>
              {CONTACT.phone ? (
                <li className="flex items-start gap-3">
                  <Phone className="mt-0.5 h-4 w-4 shrink-0 text-blue-300" aria-hidden />
                  <a
          ...
          <nav aria-label="Hizmet kategorileri">
            <h2 className="font-mono text-xs uppercase tracking-[0.2em] text-blue-300">Hizmetler</h2>
            <ul className="mt-5 space-y-2.5 text-sm">
              {categoriesWithCounts.map((category) => (
                <li key={category.key}>
                  ...
                  >
                    {category.label}
                    <span className="rounded-full border border-white/10 bg-white/[0.04] px-1.5 py-0.5 font-mono text-[10px] text-white/50 transition group-hover:border-blue-400/40 group-hover:text-blue-200">
                      {category.count}
                    </span>
                  </Link>
                </li>
            ...
            <ul className="mt-6 space-y-2.5 border-t border-white/10 pt-6 text-sm">
              {FOOTER_FEATURED_SERVICES.slice(0, 5).map((service) => (
                <li key={service.href}>
                  <Link
                    href={service.href}
                    className="inline-flex items-center gap-1.5 text-white/65 transition hover:text-blue-300"
                  >
                    {service.label}
              ...
              <li>
                <Link
                  href="/hizmetler/"
                  className="inline-flex items-center gap-1.5 font-semibold text-blue-300 transition hover:text-blue-200"
                >
                  Tüm 43 hizmet
          ...
          <nav aria-label="Hizmet verdiğimiz şehirler">
            <h2 className="font-mono text-xs uppercase tracking-[0.2em] text-blue-300">Şehirler</h2>
            ...
            <Link
              href="/sehir/"
              className="mt-5 inline-flex items-center gap-1.5 text-sm font-semibold text-blue-300 transition hover:text-blue-200"
            >
              81 ilin tamamı
              <ArrowUpRight className="h-3.5 w-3.5" aria-hidden />
            </Link>

            <h2 className="mt-8 font-mono text-xs uppercase tracking-[0.2em] text-blue-300">Kurumsal</h2>
            <ul className="mt-5 space-y-2.5 text-sm">
          ...
          {/* CTA */}
          <div>
            <h2 className="font-mono text-xs uppercase tracking-[0.2em] text-blue-300">
              Projenizi başlatın
            </h2>
            ...
            <div className="mt-6 grid gap-2.5">
              ...
              {waLink ? (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="rounded-lg border border-white/15 px-4 py-3 text-center text-sm font-semibold text-white/85 transition hover:border-sky-400/50 hover:bg-white/[0.06]"
                >
                  WhatsApp ile yazın
                </a>
              ) : (
                <a
                  href={`mailto:${CONTACT.email}`}
                  className="rounded-lg border border-white/15 px-4 py-3 text-center text-sm font-semibold text-white/85 transition hover:border-blue-400/50 hover:bg-white/[0.06]"
                >
                  {CONTACT.email}
                </a>
              )}
